...
`;
const Discuss = () => {
  const { teams, updateTeam2, updateEvent, searchEvent } = CartState();
  const [one, setOne] = useState("NA");
  const [two, setTwo] = useState("NA");
  const [three, setThree] = useState("NA");
  useEffect(() => {
    const fetchEvent = async () => {
      const fetchedEvent = await searchEvent("Mdisk");
  ...
  const setRank = (e, state, setState, points) => {
    if (e.target.value === "NA") {
      ...
      let val = temp.mfield - points;
      updateTeam2(val, temp._id);
      ...
      let val1 = temp1.mfield - points;
      updateTeam2(val1, temp1._id);
      const temp2 = teams.find((team) => {
        return team.name === e.target.value;
      });
      let val2 = temp2.mfield + points;
      updateTeam2(val2, temp2._id);
      setState(e.target.value);
    }
  };
  return (
    <>
      <Container>
        <Navbar page="Men's Discuss Throw" />
        <WinnerContainer>
          <Rank>
